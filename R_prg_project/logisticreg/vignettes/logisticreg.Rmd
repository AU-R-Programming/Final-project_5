...
### **3.1 Advanced Workflow with Categorical Data**

This workflow demonstrates how to handle datasets with categorical predictors and binary targets using the `logisticreg` package.

Step 1: Create a Sample Dataset with Categorical Variables

```{r}
set.seed(123)
data <- data.frame(
  predictor1 = rnorm(100),
  predictor2 = factor(sample(c("A", "B", "C"), 100, replace = TRUE)),
  target = factor(sample(c(0, 1), 100, replace = TRUE))
)
...
The `preprocess_data` function automatically prepares the dataset by:

Converting categorical predictors into a model matrix using one-hot encoding.

Ensuring the target variable is numeric and binary (0/1).

```{r}
# Preprocessing the data
processed <- preprocess_data(data, target_var = "target")

# Extract the processed design matrix and target variable
X <- processed$X
y <- processed$y
...
Use the `logistic_regression_with_factors` function to:

Estimate the logistic regression coefficients.

Compute bootstrap confidence intervals for the coefficients.

Predict class labels for the dataset.

Evaluate the model's performance with confusion matrix metrics.

```{r}
# Perform logistic regression and evaluate the model
logistic_regression_with_factors(data, target_var = "target")
